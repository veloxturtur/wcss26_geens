...
{ name: 'Pie', teams: ['Uruguay', 'Chile', 'Spain'] },
{ name: 'Sara', teams: ['Peru', 'Panama', 'Spain'] },
{ name: 'Stan', teams: ['Costa Rica', 'Honduras', 'Spain'] },
  ...
  try {
    ...
    if (raw) {
      ...
      return {
        ...
        players: HARD_CODED_PLAYERS,
        matches: parsed.matches?.length ? parsed.matches : generateDefaultMatches(),
        knockoutTeams: parsed.knockoutTeams ?? {},
        lastSyncAt: parsed.lastSyncAt ?? null,
        syncSource: parsed.syncSource ?? null,
        lastSyncCount: parsed.lastSyncCount ?? 0,
        lastLeaderboard: Array.isArray(parsed.lastLeaderboard) ? parsed.lastLeaderboard : [],
        rankMovement: parsed.rankMovement ?? {},
      };
    }
    ...
    if (legacy) {
      ...
      return {
        ...
        players: HARD_CODED_PLAYERS,
        matches: old.matches?.length ? old.matches : generateDefaultMatches(),
        knockoutTeams: old.knockoutTeams ?? {},
        lastSyncAt: null,
        syncSource: null,
        lastSyncCount: 0,
        lastLeaderboard: [],
